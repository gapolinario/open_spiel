...
void BasicAgnesSorelTests() {
  testing::LoadGameTest("agnes_sorel");
  testing::RandomSimTest(*LoadGame("agnes_sorel"), 100);
}
...
void TestMoveActionId() {
  ...
  for (const auto &move : valid_moves) {
    std::cout << static_cast<int>(move.ActionId()) << "; " << move.ToString()
...
int main(int argc, char **argv) {
  open_spiel::agnes_sorel::TestMoveActionId();
  open_spiel::agnes_sorel::BasicAgnesSorelTests();
}
